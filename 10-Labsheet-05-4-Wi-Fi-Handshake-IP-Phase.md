...
| **5.4.2** | Password ผิด |ไม่เกิด |ไม่เกิด |Failed | 2 (0x02)|
...
1. เหตุใดกระบวนการ **4-Way Handshake** จึงพิสูจน์ทราบรหัสผ่าน Wi-Fi ได้โดยไม่ต้องส่งรหัสผ่าน (Passphrase) ลอยไปในอากาศเลยแม้แต่แพ็กเกจเดียว?<br>
    ตอบ ใช้ระบบท้าทายด้วยค่าสุ่มและค่าแฮชยืนยันตัวตนโดยทั้งสองฝ่ายจะเอารหัสผ่านที่มีอยู่กับตัวมาคำนวณเงียบ ๆ ฝั่งใครฝั่งมัน แล้วส่งเฉพาะค่าแฮชมาเทียบกัน ถ้าค่าตรงกันแปลว่ารหัสถูก โดยไม่ต้องส่งตัวรหัสจริงออกอากาศ
2. อธิบายบทบาทและที่มาของคีย์ **PMK (Pairwise Master Key)** และ **PTK (Pairwise Transient Key)** ว่ามีความสัมพันธ์กันอย่างไรในการเข้ารหัสเฟรมข้อมูล?<br>
    ตอบ PMK คีย์แม่บท ได้มาจากการนำ Password + SSID มาปั่นรวมกัน ส่วน PTK คีย์ชั่วคราว ได้จากการเอา PMK ไปผสมกับค่าสุ่ม และ MAC Address ของทั้งสองฝ่าย เกิดเป็นคีย์ใหม่ทุกครั้งที่เชื่อมต่อ เอาไว้เข้ารหัสรับ-ส่งข้อมูลจริงในเซสชันนั้น
3. เหตุใดเมื่อเราพิมพ์ Password ผิด (ข้อ 5.4.2) ESP32 จึงยังคงได้รับ Event **`WIFI_EVENT_STA_CONNECTED`** ก่อนที่จะเกิด Event **`WIFI_EVENT_STA_DISCONNECTED`** ตามมาในภายหลัง?<br>
    ตอบ เพราะ STA_CONNECTED แจ้งเตือนแค่ว่าบอร์ดเกาะคลื่นวิทยุและตกลงสเปกอุปกรณ์ผ่านแล้ว (Phase 2 & 3) จากนั้นระบบถึงค่อยเริ่มกระบวนการเช็กรหัสผ่านจริงใน Phase 4 พอคำนวณคีย์ไม่ผ่านเพราะรหัสผิด มันจึงสั่งตัดสายและเกิด STA_DISCONNECTED ตามหลังมา
4. หากเครือข่าย Wi-Fi ไม่มี DHCP Server (ไม่มีการแจก IP อัตโนมัติ) ผลการทดลองในข้อ 5.4.1 จะหยุดอยู่ที่ขั้นตอนใด และจะไม่เกิด Event ใดขึ้น?<br>
    ตอบ ผลการทดลองจะหยุดอยู่ที่ Phase 4 (4-Way Handshake สำเร็จ) และจะไม่เกิด Event IP_EVENT_STA_GOT_IP ขึ้น
